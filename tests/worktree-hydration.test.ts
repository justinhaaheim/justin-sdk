...
// ---------------------------------------------------------------------------
// generated-input: tsconfig names a file the worktree does not have (qe6b.4 D1)
// ---------------------------------------------------------------------------

/**
 * THE BLIND SPOT THIS CLOSES, measured on nature-sounds 2026-09-12: the repo
 * has no `.worktreeinclude`, so a fresh worktree produced ZERO hydration
 * problems while `expo-env.d.ts` — gitignored, generated by the Expo CLI, and
 * named literally in the tsconfig's `include` — was absent. `signal` therefore
 * ran and relayed 16 phantom `no-unsafe-*` errors, the exact misdiagnosis this
 * module exists to prevent.
 *
 * The fixture mirrors that shape exactly: an untracked, gitignored file in the
 * primary, named literally in a tsconfig that otherwise uses globs.
 */
describe('generated-input (D1)', () => {
  const TSCONFIG = JSON.stringify(
    {
      compilerOptions: {strict: true},
      include: ['expo-env.d.ts', 'src/**/*.ts', '.expo/types/**/*.ts'],
    },
    null,
    2,
  );
  ...
  function setup(
    sb: Sandbox,
    options: {
      generatedInPrimary?: boolean;
      tsconfig?: string;
      worktreeinclude?: string;
    } = {},
  ): {primary: string; wt: string} {
    const files: Record<string, string> = {
      'tsconfig.json': options.tsconfig ?? TSCONFIG,
    };
    if (options.generatedInPrimary !== false) {
      files['expo-env.d.ts'] = '/// <reference types="expo/types" />\n';
    }
    const primary = makePrimary(sb, {
      files,
      gitignore: 'node_modules/\nexpo-env.d.ts\n.claude/worktrees/\n',
      worktreeinclude: options.worktreeinclude,
    });
    // Fixture sanity: the generated file must be UNTRACKED in the primary, or
    // git would simply check it out into the worktree and there is nothing to
    // detect. tsconfig.json, by contrast, must be tracked.
    if (options.generatedInPrimary !== false) {
      expect(git(primary, ['ls-files', '--', 'expo-env.d.ts'])).toBe('');
    }
    expect(git(primary, ['ls-files', '--', 'tsconfig.json'])).toContain(
      'tsconfig.json',
    );
    const wt = addLinkedWorktree(
      primary,
  ...
  test('a tsconfig-named file present in the primary and absent here IS reported', () => {
    const sb = track(createSandbox());
    const {wt} = setup(sb);
    expect(existsSync(join(wt, 'expo-env.d.ts'))).toBe(false);

    const status = detectWorktreeHydration(wt);
    expect(kinds(status)).toEqual(['generated-input']);
    expect(status.problems[0]?.label).toBe('expo-env.d.ts');
    // Both fix routes, named — the whole point of the message (AC 2).
    expect(status.problems[0]?.detail).toContain('.worktreeinclude');
    expect(status.problems[0]?.detail).toContain('setup-env:<LABEL>');
    expect(status.problems[0]?.detail).toContain('tsconfig.json');
  });

  test('it is ADVISORY, never blocking', () => {
    const sb = track(createSandbox());
    const {wt} = setup(sb);
    const status = detectWorktreeHydration(wt);
    expect(
      status.problems.every((problem) => !isBlockingProblem(problem)),
    ).toBe(true);
    expect(hasBlockingProblem(status)).toBe(false);
    expect(isBlockingProblem(makeProblem('generated-input'))).toBe(false);
    expect([...BLOCKING_PROBLEM_KINDS]).not.toContain('generated-input');
  });

  test('present in BOTH checkouts is not a problem (and copying it in clears the report)', () => {
    const sb = track(createSandbox());
    const {wt} = setup(sb);
    expect(kinds(detectWorktreeHydration(wt))).toEqual(['generated-input']);
    writeFileSync(join(wt, 'expo-env.d.ts'), 'anything\n');
    expect(kinds(detectWorktreeHydration(wt))).toEqual([]);
  });

  /**
   * The glob filter has to be LOAD-BEARING, not merely unreachable. A pattern
   * like `src/**\/*.ts` can never name a real file, so a fixture built only
   * from those would pass with the filter deleted. `generated/a[b].d.ts` is a
   * legal macOS filename AND a glob, so it exists in the primary, is absent
   * here, and is skipped only because the filter skips it.
   */
  test('GLOB entries are never reported, even when one names a real file', () => {
    const sb = track(createSandbox());
    const primary = makePrimary(sb, {
      files: {
        'generated/a[b].d.ts': 'declare const x: 1;\n',
        'tsconfig.json': JSON.stringify({
          include: ['**/*.ts', 'src/?.ts', '{a,b}.d.ts', 'generated/a[b].d.ts'],
        }),
      },
      gitignore: 'node_modules/\ngenerated/\n.claude/worktrees/\n',
    });
    expect(git(primary, ['ls-files', '--', 'generated/'])).toBe('');
    const wt = addLinkedWorktree(
      primary,
      join(primary, '.claude', 'worktrees', 'w1'),
      'worktree-w1',
    ...
    hydrateFixtureDeps(wt);
    // Fixture sanity: it really is present there and absent here — i.e. the
    // ONLY thing standing between this tree and a report is the glob filter.
    expect(existsSync(join(primary, 'generated', 'a[b].d.ts'))).toBe(true);
    expect(existsSync(join(wt, 'generated', 'a[b].d.ts'))).toBe(false);

    expect(kinds(detectWorktreeHydration(wt))).toEqual([]);
  });

  test('a file missing from the PRIMARY too is not reported (nothing to copy)', () => {
    const sb = track(createSandbox());
    const {wt} = setup(sb, {generatedInPrimary: false});
    expect(kinds(detectWorktreeHydration(wt))).toEqual([]);
  });

  test('a PRIMARY checkout is never reported — there is nowhere to copy from', () => {
    const sb = track(createSandbox());
    const {primary} = setup(sb);
    hydrateFixtureDeps(primary);
    // The primary HAS the file, so delete it to create the strongest possible
    // temptation: a tsconfig-named file that is absent from this very checkout.
    rmSync(join(primary, 'expo-env.d.ts'));
    const status = detectWorktreeHydration(primary);
    expect(status.isLinkedWorktree).toBe(false);
    expect(kinds(status)).toEqual([]);
  });

  test('a file already reported as worktreeinclude is NOT reported twice', () => {
    const sb = track(createSandbox());
    const {wt} = setup(sb, {worktreeinclude: 'expo-env.d.ts\n'});
    const status = detectWorktreeHydration(wt);
    // Exactly one problem, and it is the manifest one (the better message).
    expect(kinds(status)).toEqual(['worktreeinclude']);
    expect(
      status.problems.filter((problem) => problem.label === 'expo-env.d.ts'),
    ).toHaveLength(1);
  });

  test('doctor renders the file and both fix routes, without claiming the results are trustworthy (AC 2)', () => {
    const sb = track(createSandbox());
    const {wt} = setup(sb);
    const result = makeEnvHydrationChecks(wt)[0]?.check.fn?.() as {
      fix?: string;
    ...
    expect(result.message).toContain('partially unhydrated');
    expect(result.message).toContain('expo-env.d.ts');
    expect(result.message).toContain('.worktreeinclude');
    expect(result.message).toContain('setup-env:<LABEL>');
    // NOT the blocking claim…
    expect(result.message).not.toContain('PHANTOM');
    // …and NOT the ordinary advisory claim either: a missing generated input
    // was measured to degrade type-aware lint to `any`, so "still trustworthy"
    // would be false here.
    expect(result.message).not.toContain('still trustworthy');
    expect(result.message).toContain('type-aware lint results here may be');
  });

  test('the signal/setup-env advisory block drops the same false claim', () => {
    const sb = track(createSandbox());
    const {wt} = setup(sb);
    const warning = formatAdvisoryWorktreeWarning(detectWorktreeHydration(wt));
    expect(warning).toContain('partially unhydrated worktree');
    expect(warning).toContain('expo-env.d.ts');
    expect(warning).not.toContain('does not affect check results');
    expect(warning).toContain('type-aware lint results may be wrong');
  });
});

describe('literalTsconfigInputs (D1 parsing rules)', () => {
  test('takes include AND files, dropping every glob', () => {
    expect(
      literalTsconfigInputs(
        JSON.stringify({
          files: ['generated/version.d.ts'],
          include: ['expo-env.d.ts', 'src/**/*.ts'],
        }),
      ),
    ).toEqual(['expo-env.d.ts', 'generated/version.d.ts']);
  });

  test('tolerates the JSONC a tsconfig is allowed to contain', () => {
    const text = [
      '{',
      '  // the generated Expo types',
      '  "include": [',
      '    "expo-env.d.ts", /* block comment */',
      '    "src/**/*.ts",',
      '  ],',
      '}',
    ].join('\n');
    expect(literalTsconfigInputs(text)).toEqual(['expo-env.d.ts']);
  });

  /**
   * A regex-only comment stripper eats from `//` to the end of the line, which
   * here would truncate the JSON and yield `[]` — silently, and with a
   * perfectly calm empty result. The entry surviving at all is the assertion;
   * `a//b.d.ts` arrives as `a/b.d.ts` only because path normalization collapses
   * a duplicate separator, which is the same file.
   */
  test('a `//` inside a STRING is not a comment', () => {
    expect(stripJsoncExtras('{"files": ["a//b.d.ts"]}')).toBe(
      '{"files": ["a//b.d.ts"]}',
    );
    expect(
      literalTsconfigInputs('{"files": ["a//b.d.ts"], "include": []}'),
    ).toEqual(['a/b.d.ts']);
  });

  test('a comma before a bracket INSIDE a string is not a trailing comma', () => {
    expect(stripJsoncExtras('{"files": ["weird, ].d.ts"]}')).toBe(
      '{"files": ["weird, ].d.ts"]}',
    );
  });

  test('normalizes ./ and de-duplicates; drops absolute and escaping paths', () => {
    expect(
      literalTsconfigInputs(
        JSON.stringify({
          files: ['./expo-env.d.ts'],
    ...
    ).toEqual(['expo-env.d.ts']);
  });

  test('unparseable or shapeless input yields nothing rather than a guess', () => {
    expect(literalTsconfigInputs('{not json')).toEqual([]);
    expect(literalTsconfigInputs('null')).toEqual([]);
    expect(literalTsconfigInputs('{"include": "not-an-array"}')).toEqual([]);
    expect(literalTsconfigInputs('{}')).toEqual([]);
  });
});

...
- **Finish a release with `justin-sdk-publish` (home-base).** The health-notices probe caches its last remote tag lookup for `checkIntervalMinutes` (60 by default), so a release you just pushed goes unnoticed for up to an hour — including by your own repos. `justin-sdk-publish` clears that `lastCheck` after a successful push (home-base-uxwc.4); doing the tag-and-push by hand skips it.

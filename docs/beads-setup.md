...
- The BR doctor check has a built-in fallback: its `fixCommand` tries `mise install` first, then falls back to the official install script from GitHub releases
- `setup-env.ts` delegates to `doctor --fix` in remote mode, which handles this automatically

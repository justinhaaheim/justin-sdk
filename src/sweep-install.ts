...
/**
 * Add every component that is INSTALLED here but not listed in the config.
 *
 * "Installed" is `componentInstalledEvidence`, the same detector `install` uses
 * to decide what is on disk — not a guess from file names.
 *
...
export function adoptInstalledComponents(
  ...
  if (!resolved.ok) return {adopted: [], config};
  ...
  for (const name of COMPONENT_NAMES) {
    ...
    if (!componentInstalledEvidence(projectRoot, name).installed) continue;
    adopted.push(configName);
  }
  if (adopted.length === 0) return {adopted: [], config};
...
export function applyInstallPayloadConfig(
  ...
  return {adopted: adoption.adopted, changed, dropped: dead.dropped};
}
...
export function renderInstallPayloadPlan(
  ...
  lines.push(
    plan.adopted.length === 0
      ? 'adopt: nothing — the config already lists everything installed'

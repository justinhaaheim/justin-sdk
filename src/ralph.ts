 ...
 * every paragraph is paid for on every iteration of the chain. A unit test
 * caps the composed text at 6,000 characters.
 */
...
A previous session ended and left your starting instructions in bead ${bead.id}
...
export function crashBootPlan(iteration: number, subtype: string | null): BootPlan {
  return {
    kind: 'reconstruct',
    reason: `session ${iteration} ended without handing anything over (${subtype ?? 'no reason recorded'})`,
  };
}
...
export function runIteration(
  ...
  const args = [
    ...
    // 2026-09-08 (home-base-1r6d.33.4): print mode still passes --json-schema
    // above while the contract now teaches the handoff bead instead of a
    // verdict. That mismatch is accepted for now — print mode is legacy, and
    // home-base-1r6d.33.2 decides whether it survives at all. Nothing runs the
    // loop between the two beads.
    bootContract(
...
export async function runIterationAttachable(
  ...
  const banner = deps.dispatch(cwd, [
    ...
    opts.permissionMode,
    '--append-system-prompt',
    // 2026-09-08 (home-base-1r6d.33.4): the contract now tells the session to
    // hand off through a bead (D2), but this runner still reads
    // tmp/ralph-verdict.json below and still deletes it above — so a compliant
    // session writes a handoff bead the runner does not yet look at. The
    // intermediate state is deliberate and safe: home-base-1r6d.33.2 rewires
    // the runner to the bead, and nothing runs the loop before it lands.
    bootContract(
      sessionContract({blockedWaitMin: opts.blockedWaitMin, label: boot.label}),
      boot,
    ),

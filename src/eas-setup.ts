 ...
 *
 * The update/ship scripts delegate to the shared `bunx justin-sdk eas-update`
 ...
 * explicitly with `bunx justin-sdk add eas`. It self-registers `eas-setup` in
...
const EAS_SCRIPTS: ReadonlyArray<{key: string; value: string}> = [
  ...
  },
  {
    ...
    value:
      'bun run prebuild && APP_VARIANT=development bunx justin-sdk eas-update development',
  },
  {
    key: 'eas:update:preview',
    value:
      'bun run prebuild && APP_VARIANT=preview bunx justin-sdk eas-update preview',
  },
  {
